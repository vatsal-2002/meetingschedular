...
//       adjustedStartTime.setHours(parseInt(slot.start.hour), parseInt(slot.start.minute), 0);
...
const Viewlivepage = () => {
  ...
  const handleCopyLink = () => {
    ...
    const formattedTimezoneArray = timezoneArray.replace("/", "-");
    const url = `/meeting?id=${meetingId}&date=${formattedDate}&timezone=${encodeURIComponent(formattedTimezoneArray)}`;
