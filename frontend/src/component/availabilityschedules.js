...
//             const weeklyhours = [{ "day": 1, "slots": [{ "end": { "hour": "18", "minute": "00" }, "start": { "hour": "17", "minute": "00" } }] }, { "day": 2, "slots": [{ "end": { "hour": "17", "minute": "00" }, "start": { "hour": "17", "minute": "00" } }] }, { "day": 3, "slots": [{ "end": { "hour": "17", "minute": "00" }, "start": { "hour": "17", "minute": "00" } }] }, { "day": 4, "slots": [{ "end": { "hour": "17", "minute": "00" }, "start": { "hour": "17", "minute": "00" } }] }, { "day": 5, "slots": [{ "end": { "hour": "17", "minute": "00" }, "start": { "hour": "17", "minute": "00" } }] }];
//             const requestBody = {
//                 isDefault,
...
//                     // Update scheduleNames state to include the newly created schedule
//                     setScheduleNames(prevScheduleNames => [...prevScheduleNames, data.name]);

//                     // Update schedules state to include the newly created schedule
...
//                 if (response.ok) {
//                     // Update the schedules state to remove the deleted schedule
...
//                     setScheduleNames(prevNames => prevNames.filter(name => name !== selectedScheduleName));

//                     // Reset selected schedule data
...
//         const selectedScheduleData = schedules.find((schedule) => schedule.name === selectedSchedule);
...
//                 method: "PATCH", // Use PATCH to update an existing schedule
//                 headers: {
//                     "Content-Type": "application/json",
//                     Authorization: `${token}`,
//                 },
...
//                     // Update the schedules state to reflect the updated schedule
        ...
        if (token && selectedScheduleId !== null) {
            try {
                ...
                if (response.ok) {
                    ...
                    // Update the scheduleNames state without the deleted schedule name
                    const updatedNames = scheduleNames.filter((name) => name !== selectedScheduleName);
